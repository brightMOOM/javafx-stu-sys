...
public class Teacher {
    ...
    private void createView() {
        ...
        TitledPane gradeSettings = new TitledPane();
        gradeSettings.setText("教师成绩管理");
        gradeSettings.setExpanded(true);
        VBox gradeSubMenu = new VBox();
        Button gradeList = createMenuButton("  成绩管理");
        Button editGrade = createMenuButton("  编辑成绩");
        gradeSubMenu.getChildren().addAll(gradeList, editGrade);
        gradeSettings.setContent(gradeSubMenu);

        sidebar.getChildren().addAll(hiTeacher, teacherEdit, courseSettings, gradeSettings);
        ...
        gradeList.setOnAction(e -> {
            TeacherGradeManage tgm = new TeacherGradeManage(tid, this);
            displayArea.getChildren().setAll(tgm);
        });

        editGrade.setOnAction(e -> {
            // By default, just show an empty or instruction message if no data selected
            displayArea.getChildren().setAll(new Label("请先在成绩管理中选择一条记录进行编辑"));
        });
    ...
    public void showEditGrade(StuMysql.TeacherGradeData data) {
        EditGrade eg = new EditGrade(tid, tname, data);
        displayArea.getChildren().setAll(eg);
    }
