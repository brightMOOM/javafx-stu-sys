...
public class TeacherGradeManage extends VBox {
    ...
    public TeacherGradeManage(int tid, Teacher teacher) {
        ...
        searchBox.setStyle("-fx-background-color: white; -fx-background-radius: 8; -fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.1), 10, 0, 0, 0);");
        ...
        resetBtn.setOnAction(e -> {
            sidField.clear(); snameField.clear(); sFuzzyCheck.setSelected(false);
            cidField.clear(); cnameField.clear(); cFuzzyCheck.setSelected(false);
            minGradeField.clear(); maxGradeField.clear();
            loadData();
        });
        row3.getChildren().addAll(new Label("成绩下限"), minGradeField, new Label("成绩上限"), maxGradeField, new Label("选择学期"), termCombo);
    ...
    private void updatePage() {
        ...
        if (start < allGrades.size()) table.getItems().addAll(allGrades.subList(start, end));
        updatePagination();
    }
    ...
    private void updatePagination() {
        ...
        Button prev = new Button("<"); prev.setDisable(currentPage == 1);
        prev.setOnAction(e -> { currentPage--; updatePage(); });
        Button pageBtn = new Button(String.valueOf(currentPage));
        pageBtn.setStyle("-fx-background-color: #1890ff; -fx-text-fill: white;");
        Button next = new Button(">"); next.setDisable(currentPage == totalPages);
        next.setOnAction(e -> { currentPage++; updatePage(); });
